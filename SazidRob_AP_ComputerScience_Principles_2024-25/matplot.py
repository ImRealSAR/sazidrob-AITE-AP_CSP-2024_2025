...
def add_entry():
    ...
    for i in range(len(data_entries)):
        data_label = Label(root, text="Enter Data Label " + str(i+1), width=15)
        data_label.grid(row=i, column=2, sticky=W)
...
def create_graph():
    ...
    for entry in entries:
        entry_data.append(int(entry.get()))
    for data_entry in data_entries:
        data_labels.append(data_entry.get())
...
graph_button = Button(root, text="Create Graph", width=15, command=create_graph)
graph_button.grid(row=e+1, column=4)
add_row_button = Button(root, text="Add Row", width=15, command=add_entry)
add_row_button.grid(row=e+2, column=4)
delete_row_button = Button(root, text="Delete Row", width=15, command=delete_row)
delete_row_button.grid(row=e+3, column=4)
